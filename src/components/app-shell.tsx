...
export function AppShell({ children, title, eyebrow, action }: { children: ReactNode; title: string; eyebrow?: string; action?: ReactNode; }) {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(({ data }) => { const active=Boolean(data.session); setSignedIn(active); if(active){void recordSessionActivity(); heartbeat=window.setInterval(()=>void recordSessionActivity(),5*60*1000);} });
    const { data } = supabase.auth.onAuthStateChange((_event, session) => { setSignedIn(Boolean(session)); if(session)void recordSessionActivity(); });
    return () => { if(heartbeat)window.clearInterval(heartbeat); data.subscription.unsubscribe(); };
  }, []);
  useEffect(() => { if(!mobileMenuOpen)return; const onKeyDown=(event:KeyboardEvent)=>event.key==="Escape"&&setMobileMenuOpen(false); document.addEventListener("keydown",onKeyDown); document.body.classList.add("mobile-menu-locked"); return()=>{document.removeEventListener("keydown",onKeyDown);document.body.classList.remove("mobile-menu-locked");}; }, [mobileMenuOpen]);
  ...
  return <div className={`app-frame ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    <aside className="desktop-sidebar"><div className="sidebar-topline"><Link to="/" preload="intent" aria-label="Start To Up landing page"><img className="sidebar-logo" src="/brand/start-to-up-logo-primary.png" alt="Start To Up" decoding="async" /></Link><button type="button" className="sidebar-collapse-button" onClick={()=>setSidebarCollapsed(v=>!v)} aria-label={sidebarCollapsed?"Expand navigation":"Collapse navigation"} title={sidebarCollapsed?"Expand navigation":"Collapse navigation"}>{sidebarCollapsed?<PanelLeftOpen size={18}/>:<PanelLeftClose size={18}/>}<span>{sidebarCollapsed?"Expand":"Collapse"}</span></button></div>
      <nav className="desktop-sidebar-scroll" aria-label="Application navigation">{navigation.map(({to,label,icon:Icon,primary})=><Link preload="intent" key={to} to={to} title={label} className={`sidebar-link ${path===to?"active":""} ${primary?"create-link":""}`}><Icon size={21}/><span>{label}</span></Link>)}<span className="sidebar-section-label">STARTUP OS</span>{startupOsNavigation.map(({to,label,icon:Icon})=><Link preload="intent" key={to} to={to} title={label} className={`sidebar-link ${path===to?"active":""}`}><Icon size={19}/><span>{label}</span></Link>)}<span className="sidebar-section-label">COLLABORATION &amp; TRUST</span>{trustNavigation.map(({to,label,icon:Icon})=><Link preload="intent" key={to} to={to} title={label} className={`sidebar-link ${path===to?"active":""}`}><Icon size={19}/><span>{label}</span></Link>)}<span className="sidebar-section-label">MEDIA &amp; SCALE</span>{scaleNavigation.map(({to,label,icon:Icon})=><Link preload="intent" key={to} to={to} title={label} className={`sidebar-link ${path===to?"active":""}`}><Icon size={19}/><span>{label}</span></Link>)}</nav>
      <div className="sidebar-footer-zone">{!sidebarCollapsed?<div className="sidebar-trust"><img src="/brand/start-to-up-symbol.png" alt="" loading="lazy" decoding="async"/><strong>Secure company workspace</strong><span>Sessions, permissions and protected sharing are active.</span></div>:null}{signedIn?<button className="sidebar-session-action" onClick={()=>void signOut()} title="Sign out"><LogOut size={16}/><span>Sign out</span></button>:<Link to="/auth" className="sidebar-session-action" title="Sign in"><UserRound size={16}/><span>Sign in</span></Link>}<Link preload="intent" to="/" className="sidebar-back" title="Public website">← <span>Public website</span></Link></div>
    </aside>
    <div className="app-content"><header className="app-header"><div className="mobile-brand"><img src="/brand/start-to-up-symbol.png" alt="Start To Up" decoding="async"/><button type="button" className="mobile-menu-trigger" onClick={()=>setMobileMenuOpen(true)} aria-label="Open navigation menu" aria-expanded={mobileMenuOpen}><Menu size={22}/></button></div><div><span className="page-eyebrow">{eyebrow??"START TO UP"}</span><h1>{title}</h1></div><div className="app-header-actions"><label className="app-search"><Search size={18}/><input aria-label="Search Start To Up" placeholder="Search innovations"/></label><Link preload="intent" to="/app/media" aria-label="Media recommendations and notifications" className="icon-button"><Bell size={20}/><i/></Link>{action}</div></header><main className="app-main">{children}</main></div>
    <nav className="mobile-navigation" aria-label="Mobile navigation">{mobileNavigation.map(({to,label,icon:Icon,primary})=><Link preload="intent" key={to} to={to} aria-current={path===to?"page":undefined} className={`${path===to?"active":""} ${primary?"mobile-create":""}`}><span className="mobile-nav-icon"><Icon size={primary?24:21}/></span><span className="mobile-nav-label">{label}</span></Link>)}</nav>
    {mobileMenuOpen?<div className="mobile-menu-layer" role="dialog" aria-modal="true" aria-label="Start To Up navigation"><button type="button" className="mobile-menu-backdrop" aria-label="Close menu" onClick={()=>setMobileMenuOpen(false)}/><section className="mobile-menu-sheet"><header><div><span>START TO UP</span><strong>Everything you need to build.</strong></div><button type="button" onClick={()=>setMobileMenuOpen(false)} aria-label="Close navigation menu"><X/></button></header><div className="mobile-menu-grid">{mobileMoreNavigation.map(({to,label,icon:Icon,description})=><Link preload="intent" key={to} to={to} className={path===to?"active":""}><Icon/><div><strong>{label}</strong><span>{description}</span></div></Link>)}</div><footer><Link preload="intent" to="/startup-playbook">Startup playbook</Link><Link preload="intent" to="/">Public website</Link></footer></section></div>:null}
