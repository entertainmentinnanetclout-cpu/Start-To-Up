        ...
        <nav aria-label="Application navigation">
          {navigation.map(({ to, label, icon: Icon, primary }) => (
            <Link
              key={to}
              to={to}
              className={`sidebar-link ${path === to ? "active" : ""} ${primary ? "create-link" : ""}`}
            >
              <Icon size={21} />
              <span>{label}</span>
            </Link>
      ...
      <div className="app-content">
        <header className="app-header">
          <div className="mobile-brand">
            <img src="/brand/start-to-up-symbol.png" alt="Start To Up" />
            <Menu size={22} />
          </div>
          <div>
            <span className="page-eyebrow">{eyebrow ?? "START TO UP"}</span>
            <h1>{title}</h1>
          </div>
