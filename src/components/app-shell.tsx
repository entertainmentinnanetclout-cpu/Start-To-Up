...
export function AppShell({ children, title, eyebrow, action }: { children: ReactNode; title: string; eyebrow?: string; action?: ReactNode; }) {
  ...
  useEffect(() => { setMobileMenuOpen(false); setHelpOpen(false); recordRecentWork(path, title); }, [path, title]);
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("start-to-up:operating-preferences", onPreference); window.removeEventListener("start-to-up:saved-views", onViews); };
  }, []);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape") { setCommandOpen(false); setMobileMenuOpen(false); setHelpOpen(false); }
    };
  ...
  return <div className={`app-frame ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    <aside className="desktop-sidebar">
      ...
      <div className="sidebar-footer-zone">
        {!sidebarCollapsed && activeWorkspace ? <div className="sidebar-trust"><Building2/><strong className="operating-workspace-sidebar-copy">{activeWorkspace.name}</strong><span className="operating-workspace-sidebar-copy">Active company workspace</span></div> : null}
    ...
    <div className="app-content">
      <header className="app-header">
        ...
        <div className="app-header-actions">
          {signedIn && workspaces.length ? <label className="operating-workspace-switcher"><span>ACTIVE COMPANY</span><select value={activeWorkspace?.organization_id || ""} onChange={(event) => changeWorkspace(event.target.value)}>{workspaces.map((workspace) => <option key={workspace.organization_id} value={workspace.organization_id}>{workspace.name}</option>)}</select></label> : null}
