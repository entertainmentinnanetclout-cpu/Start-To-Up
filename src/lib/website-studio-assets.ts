...
import { getStudioAccessState } from "./website-studio-access";
...
export type UploadedWebsiteStudioAsset = {
  id?: string; path: string; publicUrl: string; originalName: string; contentType: string; bytes: number; slot?: WebsiteStudioAssetSlot;
  altText?: string; focalX?: number; focalY?: number; checksum?: string; transform?: Record<string, unknown>; source?: string;
};
...
async function requireApprovedUploadAccess() {
  const state = await getStudioAccessState();
  if (!state.signedIn) throw new Error("SIGN_IN_REQUIRED");
...
export async function uploadWebsiteStudioAsset(file: File, slot: WebsiteStudioAssetSlot, projectId?: string, metadata?: { altText?: string; focalX?: number; focalY?: number; transform?: Record<string, unknown>; source?: string }): Promise<UploadedWebsiteStudioAsset> {
  validate(file, slot);
  await requireApprovedUploadAccess();
  ...
  if (!data.publicUrl) { await supabase.storage.from(BUCKET).remove([path]).catch(() => undefined); throw new Error("PUBLIC_URL_FAILED"); }
  const database = supabase as any;
  const { data: row, error: recordError } = await database.from("website_studio_assets").insert({
    owner_id: authData.user.id, project_id: projectId || null, slot, storage_path: path, public_url: data.publicUrl, original_filename: file.name,
    mime_type: file.type, byte_size: file.size, alt_text: metadata?.altText || "", focal_x: metadata?.focalX ?? .5, focal_y: metadata?.focalY ?? .5,
    checksum_sha256: checksum || null, source: metadata?.source || "upload", transform_config: metadata?.transform || {},
  }).select("*").single();
  if (recordError) { await supabase.storage.from(BUCKET).remove([path]).catch(() => undefined); throw recordError; }
...
function rowToAsset(row: any): UploadedWebsiteStudioAsset {
  return { id: row.id, path: row.storage_path, publicUrl: row.public_url, originalName: row.original_filename, contentType: row.mime_type, bytes: Number(row.byte_size || 0), slot: row.slot, altText: row.alt_text || "", focalX: Number(row.focal_x ?? .5), focalY: Number(row.focal_y ?? .5), checksum: row.checksum_sha256 || "", transform: row.transform_config || {}, source: row.source || "upload" };
}
export async function listWebsiteStudioAssets(limit = 72, projectId?: string): Promise<UploadedWebsiteStudioAsset[]> {
  const { data: authData, error: authError } = await supabase.auth.getUser(); if (authError || !authData.user) return [];
  const database = supabase as any;
  ...
  const { data, error } = await query; if (error) throw error; return (data || []).map(rowToAsset);
}
export async function updateWebsiteStudioAsset(assetId: string, patch: { altText?: string; focalX?: number; focalY?: number; transform?: Record<string, unknown> }) {
  ...
  const database = supabase as any; const payload: Record<string, unknown> = {};
  if (patch.altText !== undefined) payload.alt_text = patch.altText; if (patch.focalX !== undefined) payload.focal_x = patch.focalX; if (patch.focalY !== undefined) payload.focal_y = patch.focalY; if (patch.transform !== undefined) payload.transform_config = patch.transform;
  const { data, error } = await database.from("website_studio_assets").update(payload).eq("id", assetId).eq("owner_id", authData.user.id).select("*").single(); if (error) throw error; return rowToAsset(data);
}
export async function saveWebsiteStudioAssetVariant(assetId: string, projectId: string | undefined, variant: { key: string; file: File; width: number; height: number }) {
  await requireApprovedUploadAccess();
  const { data: authData } = await supabase.auth.getUser(); if (!authData.user) throw new Error("SIGN_IN_REQUIRED");
  const filename = cleanFilename(variant.file.name); const path = `${authData.user.id}/variants/${assetId}/${variant.key}-${filename}`;
  const { error: uploadError } = await supabase.storage.from(BUCKET).upload(path, variant.file, { upsert: true, cacheControl: "31536000", contentType: variant.file.type }); if (uploadError) throw uploadError;
  const publicUrl = supabase.storage.from(BUCKET).getPublicUrl(path).data.publicUrl; const database = supabase as any;
...
export async function removeWebsiteStudioAsset(asset: UploadedWebsiteStudioAsset) {
  if (!asset.id) return; await requireApprovedUploadAccess();
  const { data: authData } = await supabase.auth.getUser(); if (!authData.user) throw new Error("SIGN_IN_REQUIRED"); const database = supabase as any;
  const { error } = await database.from("website_studio_assets").delete().eq("id", asset.id).eq("owner_id", authData.user.id); if (error) throw error; await supabase.storage.from(BUCKET).remove([asset.path]).catch(() => undefined);
}
export function websiteStudioAssetErrorMessage(error: unknown) {
  ...
  if (message === "APPROVAL_REQUIRED") return "Your Website Studio account must be approved before uploads are enabled.";
