...
console.log(`Startup OS backend contract passed: ${migrations.length} migrations with unique versions, Phases 0–10 present, role-helper compatibility explicit, Phase 1 anon grants removed, Phase 2 finance backend complete, reconciliation/health controls active in source, workspace SECURITY DEFINER RPCs hardened, ${edgeFunctions.length} server gateways present, assistant metric schema aligned, and browser service-role leakage blocked.`);
