...
const phaseFiles:Record<number,string[]>={
  ...
  1:['20260827100000_startup_os_phase1_validation_intelligence.sql','20260827101000_startup_os_phase1_meta_ads_evidence.sql','20260902072500_startup_os_phase1_health_registry_fix.sql'],
...
console.log(`Startup OS backend contract passed: ${migrations.length} migrations with unique versions, Phases 0–10 present, role-helper compatibility explicit, Phase 2 finance backend complete, reconciliation/health controls active in source, ${edgeFunctions.length} server gateways present, assistant metric schema aligned, and browser service-role leakage blocked.`);
