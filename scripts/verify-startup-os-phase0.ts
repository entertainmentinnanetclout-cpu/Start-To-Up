...
const executableSql = sql
  .replace(/\/\*[\s\S]*?\*\//g, " ")
  .replace(/^\s*--.*$/gm, " ");
if (/create\s+type\s+if\s+not\s+exists/i.test(executableSql)) throw new Error("Phase 0 migration contains unsupported CREATE TYPE IF NOT EXISTS syntax");
...
if (/grant\s+(?:select|all|insert|update|delete)[\s\S]{0,120}startup_os_provider_credentials[\s\S]{0,120}to\s+authenticated/i.test(executableSql)) {
